...
var TaskQuery = struct {
	...
	Select: `SELECT id, mentor_id, student_id, title, description, deadline, status, grade, feedback, submission_url, criteria, created_at, updated_at FROM tasks `,
	SelectDTO: `SELECT t.id, t.mentor_id, mentor.name AS mentor_name, t.student_id, student.name AS student_name, t.title, t.description, t.deadline, t.status, t.grade, t.feedback, t.submission_url, t.criteria, t.created_at, t.updated_at, tf.id AS latest_file_id, tf.file_url AS latest_file_url
		FROM tasks t
	...
	Insert: `INSERT INTO tasks (id, mentor_id, student_id, title, description, deadline, status, submission_url, criteria, created_at, updated_at)
		VALUES (:id, :mentor_id, :student_id, :title, :description, :deadline, :status, :submission_url, :criteria, :created_at, :updated_at) RETURNING id`,
	Update:             `UPDATE tasks SET mentor_id = :mentor_id, student_id = :student_id, title = :title, description = :description, deadline = :deadline, status = :status, grade = :grade, feedback = :feedback, submission_url = :submission_url, criteria = :criteria, updated_at = :updated_at WHERE id = :id`,
	UpdateSubmitted:    `UPDATE tasks SET status = :status, submission_url = :submission_url, updated_at = :updated_at WHERE id = :id`,
	UpdateGrade:        `UPDATE tasks SET status = :status, grade = :grade, feedback = CONCAT(COALESCE(feedback, ''), CAST(:feedback AS text)), updated_at = :updated_at WHERE id = :id`,
	ExistByStudentID:   `SELECT id FROM tasks`,
	ExistByMentorID:    `SELECT id FROM tasks`,
	ResolveByID:        `SELECT id, mentor_id, student_id, title, description, deadline, status, grade, feedback, submission_url, criteria, created_at, updated_at FROM tasks`,
